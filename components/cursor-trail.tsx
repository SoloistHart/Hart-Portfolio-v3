...
export function CursorTrail({
  ...
  useEffect(() => {
    ...
    const pointers = [new PointerData()];
    ...
    function getWebGLContext(cvs: HTMLCanvasElement) {
      ...
      const isWebGL2 = !!(glCtx as WebGL2RenderingContext)?.fenceSync;
      ...
      if (isWebGL2) {
        ...
        halfFloatExt = glCtx!.getExtension("OES_texture_half_float") as { HALF_FLOAT_OES: number } | null;
    ...
    const blit = (() => {
      ...
      return (target: FBOData | null, clear = false) => {
        if (target == null) {
    ...
    let dye: DoubleFBOData;
    let velocity: DoubleFBOData;
    let divergence: FBOData;
    let curl: FBOData;
    let pressure: DoubleFBOData;
    ...
    function createFBO(w: number, h: number, internalFormat: number, format: number, type: number, param: number): FBOData {
    ...
    function createDoubleFBO(w: number, h: number, internalFormat: number, format: number, type: number, param: number): DoubleFBOData {
    ...
    function resizeFBO(target: FBOData, w: number, h: number, internalFormat: number, format: number, type: number, param: number): FBOData {
    ...
    function resizeDoubleFBO(target: DoubleFBOData, w: number, h: number, internalFormat: number, format: number, type: number, param: number): DoubleFBOData {
    ...
    function render(target: FBOData | null) {
      gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
      gl.enable(gl.BLEND);
      drawDisplay(target);
    }

    function drawDisplay(target: FBOData | null) {
    ...
    function generateColor() {
      // Teal accent palette — varies hue slightly around 170-180 (cyan/teal)
      const hue = 0.47 + Math.random() * 0.06; // ~170°-178° range
